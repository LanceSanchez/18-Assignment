...
def test():
	return jsonify({'message' : 'Success! Please use the python shell or postman for commands. For python shell you can return the json by typing <var_name in request>.json(). \n Commands: for get is requests.get("http://127.0.0.1:5000/rosplayer") or when specifying a player requests.get("http://127.0.0.1:5000/rosplayer=<codeName>"). For post is requests.post("http://127.0.0.1:5000/rosplayer", json = {"<attributes>"}). Put is requests.put("http://127.0.0.1:5000/rosplayer=<codeName of player to update>", json = {"<attributes>"}) and finally delete is request.delete("http://127.0.0.1:5000/rosplayer=<codeName of player to delete>")'})
